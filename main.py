...
async def create_task(task: InTask):
    ...
    while existing_task:
        existing_task, _idx = find_task_by_id(global_tasks, new_task_id)
        if existing_task:
            new_task_id = generate_random_id()
...
async def get_tasks(status: Optional[TasksStatus] = None, task_id: Optional[str] = Query(None, min_length=11, max_length=11)):
    if task_id:
        filtered_task, _idx = find_task_by_id(global_tasks, task_id)
        if filtered_task:
